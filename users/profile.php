...
if (!is_dir($profileUploadDir)) mkdir($profileUploadDir, 0755, true);
...
if ($bc) $bookingCount = $bc->fetch_assoc()['c'];

$reviewCount = 0;
$rc = $conn->query("SELECT COUNT(*) c FROM reviews WHERE user_id = $user_id");
if ($rc) $reviewCount = $rc->fetch_assoc()['c'];
        ...
        <div id="profileView">
            <div class="bg-white rounded-xl shadow-sm border border-brand-200/50 overflow-hidden">
                <div class="bg-brand-200 px-4 py-4 text-center">
                    ...
                    <?php else: ?>
                        <div class="w-14 h-14 mx-auto rounded-full bg-white/80 flex items-center justify-center text-brand-900 text-lg font-bold shadow-sm mb-1.5">
                ...
                <div class="p-4 space-y-3">
                    <div class="flex items-center gap-3 p-3 bg-brand-50 rounded-lg">
                        <div class="w-8 h-8 rounded-full bg-brand-200 flex items-center justify-center shrink-0">
                            <svg class="w-4 h-4 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="text-[10px] text-gray-400 uppercase">Full Name</div>
                            <div class="text-sm font-semibold text-brand-900 truncate"><?= htmlspecialchars($user['name'] ?? '—') ?></div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-brand-50 rounded-lg">
                        <div class="w-8 h-8 rounded-full bg-brand-200 flex items-center justify-center shrink-0">
                            <svg class="w-4 h-4 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="text-[10px] text-gray-400 uppercase">Email</div>
                            <div class="text-sm font-semibold text-brand-900 truncate"><?= htmlspecialchars($user['email'] ?? '—') ?></div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-brand-50 rounded-lg">
                        <div class="w-8 h-8 rounded-full bg-brand-200 flex items-center justify-center shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-sm font-semibold text-brand-900 truncate"><?= htmlspecialchars($user['phone'] ?? '—') ?></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-4 pb-4">
                    <button type="button" onclick="enableEdit()"
                        ...
                        <svg class="w-3.5 h-3.5 inline mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        Edit Profile
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-brand-200/50 overflow-hidden">
                <div class="bg-brand-200 px-4 py-4 text-center">
                    <div class="relative inline-block mb-1.5">
                        <?php if (!empty($user['image'])): ?>
                            <img id="editProfilePreview" src="../uploads/profiles/<?= htmlspecialchars($user['image']) ?>?t=<?= time() ?>"
                                class="w-14 h-14 rounded-full object-cover border-2 border-white/80 shadow-sm">
                        <?php else: ?>
                            <div id="editProfilePreview" class="w-14 h-14 rounded-full bg-white/80 flex items-center justify-center text-brand-900 text-lg font-bold shadow-sm">
                                <?= htmlspecialchars($initials ?: '?') ?>
                            </div>
                        <?php endif; ?>
                        ...
                        <input type="file" name="profile_image" id="profileImageInput" accept="image/*" class="hidden" onchange="previewProfileImage(this)">
                    </div>
