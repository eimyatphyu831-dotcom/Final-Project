...
<!-- TITLE -->
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 flex flex-wrap gap-3">
        ...
        <button onclick="filterEvents('corporate')" class="px-4 py-2 text-sm rounded-full <?= btnClass($filterType, 'corporate') ?>">
            Corporate
        </button>

        <button onclick="filterEvents('wedding')" class="px-4 py-2 text-sm rounded-full <?= btnClass($filterType, 'wedding') ?>">
            Wedding
        </button>

        <button onclick="filterEvents('birthday')" class="px-4 py-2 text-sm rounded-full <?= btnClass($filterType, 'birthday') ?>">
            Birthday
        </button>

        <button onclick="filterEvents('music')" class="px-4 py-2 text-sm rounded-full <?= btnClass($filterType, 'music') ?>">
            Music
        </button>
                ...
                <div class="event-card bg-[#f7f5fa] p-4 rounded-[2rem] border border-slate-200/60 shadow-sm flex flex-col justify-between hover:shadow-md transition duration-300"
                    ...
                    <div>
                        <div class="relative w-full h-52 rounded-2xl overflow-hidden mb-5">
                            ...
                            <span class="absolute bottom-4 left-4 <?= $badgeClass ?> text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-md backdrop-blur-sm">
                                <?= $label ?>
                            </span>
                        </div>
