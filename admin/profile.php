...
<div class="flex h-screen">
   ...
   <?php include 'sidebar.php'; ?>
    ...
    <div class="flex-1 flex flex-col ml-64">
        ...
        <?php if ($message): ?>
            <div class="bg-green-100 text-green-700 px-4 py-3 rounded-xl text-sm mb-4 border border-green-200"><?= $message ?></div>
        <?php endif; ?>
        <?php if ($error): ?>
            <div class="bg-red-100 text-red-700 px-4 py-3 rounded-xl text-sm mb-4 border border-red-200"><?= $error ?></div>
        <?php endif; ?>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-4 flex flex-col items-center text-center border-b border-gray-100">
                    <div class="relative group mb-2">
                        <div class="w-14 h-14 rounded-full overflow-hidden border-4 border-gray-100 <?= $avatar ? '' : 'bg-purple-100 flex items-center justify-center' ?>">
                            <?php if ($avatar): ?>
                                <img src="<?= $avatar ?>?t=<?= time() ?>" alt="Profile" class="w-full h-full object-cover">
                            <?php else: ?>
                    ...
                    <div class="flex flex-col items-center gap-1.5">
                        <form method="POST" enctype="multipart/form-data" id="imageForm">
                            <label class="px-3 py-1.5 bg-purple-600 text-white rounded-xl text-xs hover:bg-purple-700 cursor-pointer transition-all inline-block">
                                <i class="fa-solid fa-camera mr-1"></i> Choose Image
                                <input type="file" name="profile_image" accept="image/*" class="hidden" onchange="document.getElementById('imageForm').submit()">
                            </label>
                            <input type="hidden" name="upload_image" value="1">
                        </form>
                        <?php if ($avatar): ?>
                            <form method="POST" onsubmit="return confirm('Remove profile image?')">
                                <button type="submit" name="remove_image" value="1" class="text-xs text-red-500 hover:text-red-700 transition-all">
                ...
                <div class="p-4">
                    ...
                    <form method="POST" class="space-y-1.5">
                        ...
                        <div>
                            <label class="block text-[10px] font-medium text-gray-700">New Password <span class="text-gray-400 font-normal">(leave blank to keep current)</span></label>
                            <input type="password" name="password" placeholder="••••••••"
                                   class="w-full px-2.5 py-1 border border-gray-300 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-400">
                        </div>
                        <div>
                            <label class="block text-[10px] font-medium text-gray-700">Confirm New Password</label>
                            <input type="password" name="confirm_password" placeholder="••••••••"
                                   class="w-full px-2.5 py-1 border border-gray-300 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-400">
                        </div>
                        <div class="flex gap-2">
                            <button type="submit" name="update_profile" value="1" class="px-3 py-1 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all text-xs font-medium">Save Changes</button>
                            <a href="dashboard.php" class="px-3 py-1 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all text-xs font-medium">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
