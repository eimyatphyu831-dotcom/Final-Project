...
<div class="flex h-screen">
    ...
    <div class="flex-1 flex flex-col ml-64">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <?php if ($viewUser): ?>
            <div id="viewModal" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 py-10">
                <div class="bg-white rounded-2xl shadow-xl border border-gray-200 p-6 w-full max-w-3xl mx-4 relative">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-800">Customer Details</h3>
                        <a href="customers.php<?= $search ? "?search=$search" : '' ?>" class="text-gray-400 hover:text-gray-600 text-xl"><i class="fa-solid fa-xmark"></i></a>
                    </div>
                    ...
                    <div class="flex items-center gap-4 pb-4 border-b border-gray-100 mb-4">
                        ...
                        <div>
                            <h4 class="text-lg font-semibold text-gray-800"><?= htmlspecialchars($viewUser['name']) ?></h4>
                            <p class="text-sm text-gray-500"><?= htmlspecialchars($viewUser['email']) ?></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-4 mb-6">
                        <div>
                            <label class="block text-xs font-medium text-gray-400 uppercase tracking-wide">Phone</label>
                            <p class="text-sm text-gray-800 mt-1"><?= htmlspecialchars($viewUser['phone'] ?? '—') ?></p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-400 uppercase tracking-wide">Member Since</label>
                            <p class="text-sm text-gray-800 mt-1"><?= date('F j, Y', strtotime($viewUser['created_at'])) ?></p>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                ...
                                <tr class="border-t hover:bg-gray-50">
                                    <td class="p-2 font-medium text-gray-800"><?= htmlspecialchars($bk['event_name']) ?></td>
                                    ...
                                    <td class="p-2 text-gray-600"><?= date('M j, Y', strtotime($bk['event_date'])) ?></td>
                                    ...
                                    <td class="p-2">
                                        ...
                                        ?>
                                        <span class="px-2 py-0.5 text-xs rounded-full <?= $badge ?>"><?= $bk['status'] ?></span>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <div class="flex justify-end pt-4">
                        <a href="customers.php<?= $search ? "?search=$search" : '' ?>" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition-all">Close</a>
                    </div>
                </div>
            </div>
            <?php endif; ?>
