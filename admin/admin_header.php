    ...
    <div class="flex items-center gap-6">
        ...
        <div class="relative" id="adminNotifDropdown">
            ...
            <div id="adminNotifMenu"
                ...
                <div class="p-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <button id="adminMarkAllRead" class="text-xs text-purple-600 hover:text-purple-800 cursor-pointer">Mark all read</button>
                </div>
        ...
        <div class="relative">
            <button id="profileBtn" class="flex items-center gap-3 border-l pl-6 border-gray-200 cursor-pointer">
                <div class="w-10 h-10 rounded-full overflow-hidden <?= $adminAvatar ? '' : 'bg-purple-100 flex items-center justify-center' ?>">
                    <?php if ($adminAvatar): ?>
                        <img src="<?= $adminAvatar ?>?t=<?= time() ?>" alt="Admin profile" class="w-full h-full object-cover">
                    <?php else: ?>
                        <i class="fa-solid fa-user text-purple-500 text-sm"></i>
                    <?php endif; ?>
                </div>
                <div class="text-left">
                    <h4 class="text-sm font-semibold text-gray-400 leading-none"><?= htmlspecialchars($_SESSION['user_name'] ?? 'Admin') ?></h4>
                </div>
            </button>
            <div id="profileDropdown"
                class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50">
                <a href="profile.php"
                    class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-all"><i class="fa-solid fa-user text-purple-500"></i> My Profile</a>
                <a href="../auth/logout.php"
                    class="flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-all"><i class="fa-solid fa-right-from-bracket"></i> Logout</a>
            </div>
        </div>
    </div>
</header>
