    ...
    <div class="flex-1 flex flex-col ml-64">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div id="viewModal" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 py-10">
                <div class="bg-white rounded-2xl shadow-xl border border-gray-200 p-6 w-full max-w-2xl mx-4 relative">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-800">Message Details</h3>
                        <a href="contact_messages.php<?= $search ? "?search=$search" : '' ?>" class="text-gray-400 hover:text-gray-600 text-xl"><i class="fa-solid fa-xmark"></i></a>
                    </div>
                    ...
                    <div class="flex items-center gap-4 pb-4 border-b border-gray-100 mb-4">
                        ...
                        <div>
                            <h4 class="text-lg font-semibold text-gray-800"><?= htmlspecialchars($viewMessage['name']) ?></h4>
                            <p class="text-sm text-gray-500"><?= htmlspecialchars($viewMessage['email']) ?></p>
                            <p class="text-xs text-gray-400 mt-1"><?= date('F j, Y \a\t g:i A', strtotime($viewMessage['created_at'])) ?></p>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="mb-3">
                            <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Event Type</span>
                            <p class="text-sm text-gray-800 mt-1"><?= htmlspecialchars($viewMessage['event_type']) ?></p>
                        </div>
                        <?php endif; ?>
                        <div>
                            <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Message</span>
                            <p class="text-sm text-gray-800 mt-2 bg-gray-50 p-4 rounded-xl leading-relaxed"><?= nl2br(htmlspecialchars($viewMessage['message'])) ?></p>
                        </div>
                    </div>
                    ...
                    <?php foreach ($messages as $m): ?>
                        <tr class="border-t hover:bg-gray-50 <?= !$m['is_read'] ? 'bg-purple-50/50' : '' ?>">
                            ...
                            <td class="p-3">
                                ...
                                <?php else: ?>
                                    <span class="px-3 py-1 text-xs rounded-full bg-purple-100 text-purple-700 font-medium">New</span>
                                <?php endif; ?>
                            </td>
